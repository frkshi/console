...
{
    internal class AlertDataExportQueueSaver
    {
        ...
        /// 报警数据存储管理器
        /// </summary>
        private AlertDataExportManager _manager;             
        ...
        public AlertDataExportQueueSaver(AlertDataExportManager manager)
        {
            _logger = new MtuLog();
            _manager = manager;
        ...
        private void SaveData(AlertData[] data)
        {
            if (_manager.NeedBackup)
            {
                ...
                {
                    var repository = _manager.TargetPersistenceContext.GetRepository() as SqlServer.SqlServerAlertDataExportRepository;
        ...
        private void SaveAlertDetail(AlertDataDetail[] datas)
        {
            ...
            if (_manager.NeedBackup)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        if (!repository.InsertAlertDetail(datas))
                        {
                            SubDbMonitor.DBErrorCount++;
                            _manager.BackupPersistenceContext.GetRepository().InsertAlertDetail(datas);
                            _hasBackupData = true;
